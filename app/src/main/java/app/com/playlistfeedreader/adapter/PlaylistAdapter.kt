...
class PlaylistAdapter(private val callback: ((obj: ItemModel) -> Unit)? = null) : RecyclerView.Adapter<PlaylistAdapter.MyViewHolder>() {
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        Glide.with(holder.itemView.context).load(item.snippet?.thumbnails?.standard?.url).into(holder.thumbnail)

        item.snippet?.title?.let {  holder.title.text = it }

        val fullDescription = item.snippet?.description
        if (fullDescription!!.contains("\n")){
            ...
            item.snippet?.description?.let { holder.description.text = it  }
        }
